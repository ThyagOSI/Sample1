...
void sortArray(int* A, int n, int (*swapptr)(int,int)) // using bubble sort here
{
	int i,j,temp;
	for (i =0; i< n; i++)
	{
		for (j =0; j < n-1 -i; j++)
		{
			if (swapptr(A[j],A[j+1])) //function call from its pointer
			{
				temp = A[j];
				A[j] = A[j+1];
				A[j+1] = temp;
...
int recBinSearch(int A[], int low, int high, int val)
{
	...
	if (A[mid] > val ) return BinSearch(A, low, mid - 1, val); // high = mid -1
	else if (A[mid] < val) return BinSearch(A, mid + 1, high, val); // low = mid +1 
	else return mid;
}
...
int main (void)
{
    ...
    /*int (*swapptr)(int,int)  = swapType; pointer to a function, so 
	...
	//i = BinSearch(Arr, 0,n-1,val);
	i = iterBinSearch(Arr, n-1, val);
