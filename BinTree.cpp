...
void PrintPostOrder(BSTNode* ptr) //left -right - right
{
	if (ptr == NULL) return;
...
void PrintAllPathsArr(int paths[], int len)
{
    cout << endl;
...
void PrintPathToNode2(BSTNode* root, int val, int paths[], int i)
{
    if (root == NULL) return;
    paths[i] = root->data; i++;
    if (root->data == val) PrintAllPathsArr(paths, i);
...
{
//    cout << endl << "sanity \t";
//    for (int j =0; j <=index; j++) cout << nodesarr[j] << " ";
//    cout << endl;
